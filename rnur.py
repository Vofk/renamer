...
def getdate(file_path):
    ...
    dates.sort()
    filedate = datetime.fromtimestamp(dates[0]).strftime("%d%m%y")
    return filedate
...
if __name__ == '__main__':
    ...
    for f in os.listdir(base_path):
        ...
        if re.match(r'\bD\b', nameparts[0]):  # ищем файлы начинающиеся с "D"
            newparts[1] = 'UR'
            delparts.append(nameparts[0])

        if re.match(r'\bV\b', nameparts[0]):  # ищем файлы начинающиеся с "V"
            newparts[1] = 'VS'
            delparts.append(nameparts[0])
        ...
        for part in nameparts:
            ...
            if re.match(r'\d\d\d\d', part):  # поиск возможной даты
                ...
                print('part = ',
                      part if date_in_name < datetime.strptime(datefile, "%d%m%y") else datefile)
                if newparts[2] == 'DATE':
                    if date_in_name < datetime.strptime(datefile, "%d%m%y"):
                        newparts[2] = part + '22'
                    else:
                        newparts[2] = datefile
                delparts.append(part)
